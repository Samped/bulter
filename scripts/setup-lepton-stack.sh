#!/usr/bin/env bash
# Install ARC CLI, Circle CLI, and circle-agent reference for Butler development.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"

echo "==> Butler stack setup"
echo ""

# --- ARC CLI (arc-canteen) ---
if command -v arc-canteen >/dev/null 2>&1; then
  echo "ok: arc-canteen already installed"
else
  if command -v uv >/dev/null 2>&1; then
    ...
    echo "warn: uv not found. Install uv (https://docs.astral.sh/uv/) then run:"
...
if command -v forge >/dev/null 2>&1; then
  ...
  echo "warn: Foundry not installed — run npm run delegation:bootstrap after foundryup"
fi
...
if command -v arc-canteen >/dev/null 2>&1; then
  ...
  if [[ -n "$RPC_URL" && "$RPC_URL" == http* ]]; then
    if [[ -f .env ]]; then
      ...
      echo "ok: ARC_TESTNET_RPC set from arc-canteen in .env"
    else
